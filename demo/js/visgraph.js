...
;(function(){
  ...
  !function (DGraph) {
    ...
    var VisualGraph = function(container,config){
        ...
        this.defaultConfig={
            ...
            link:{ //连线的默认配置
                ...
                linkColorType:0,//连线颜色类型 0:继承source颜色
                color:'10,10,10', //连线颜色
                alpha:1,  // 连线透明度
                lineWidth:3 //连线宽度
            },
        ...
        if(config == null){
            ...
            this.config = this.deepObjectMerge(this.config,this.defaultConfig);
        }
        ...
        this.defaultNodeSize = this.config.node.size;
        this.defaultNodeColor =this.config.node.color;
        this.defaultLabelColor =this.config.node.label.color;
    ...
    VisualGraph.prototype.deepObjectMerge = function(FirstOBJ, SecondOBJ) {
        var self = this;
        for (var key in SecondOBJ) {
            FirstOBJ[key] = FirstOBJ[key] && FirstOBJ[key].toString() === "[object Object]" ?
                self.deepObjectMerge(FirstOBJ[key], SecondOBJ[key]) : FirstOBJ[key] = SecondOBJ[key];
        }
        return FirstOBJ;
    };
  ...
  VisualGraph.prototype.newEdge = function(source,target){
    ...
    link.click(function(event){
        if(self.config.hasOwnProperty('onLinkClick')){
            var onLinkClick = self.config['onLinkClick'];
            if(typeof onLinkClick === 'function'){
              onLinkClick(event,this);
            }
        }
    });
    return link;

...
(function() {
		...
		function c(container) {
			...
			function l(a) {
				...
				n.dispatchEventToScenes("mousewheel", b), n.dispatchEvent("mousewheel", b), null != n.wheelZoom && (a.preventDefault ?
					a.preventDefault() : (a = a || window.event, a.returnValue = !1), 1 == n.thumbnail.visible && n.thumbnail.update()
				)
			}
			...
			function m(b) {
				a.util.isIE || !window.addEventListener ? (b.onmouseout = f, b.onmouseover = e, b.onmousedown = g, b.onmouseup = h,
					...
					h) : (b.addEventListener("mouseout", f), b.addEventListener("mouseover", e), b.addEventListener("mousedown", g),
					b.addEventListener("mouseup", h), b.addEventListener("mousemove", i), b.addEventListener("click", j), b.addEventListener(
		...
		function f(b, c, g) {
			...
			this.initialize = function(b, c, d) {
				...
				this.paintPath = function(a, b) {
					...
					for (var c = 1; c < b.length; c++) null == this.dashedPattern ? a.lineTo(b[c].x, b[c].y) : a.DGraphDashedLineTo(b[
						c - 1].x, b[c - 1].y, b[c].x, b[c].y, this.dashedPattern);
		...
		function g(a, b, c) {
			this.initialize = function() {
				...
				this.getStartPosition = function() {
					...
					return "horizontal" == this.direction ? this.target.cx > a.x ? a.x += this.source.width / 2 : a.x -= this.source.width /
						2 : this.target.cy > a.y ? a.y += this.source.height / 2 : a.y -= this.source.height / 2, a
				},
				this.getEndPosition = function() {
					var a = {
						x: this.target.cx,
					...
					return "horizontal" == this.direction ? this.source.cy < a.y ? a.y -= this.target.height / 2 : a.y += this.target
						.height / 2 : a.x = this.source.cx < a.x ? this.target.x : this.target.x + this.target.width, a
				},
				...
				this.paintText = function(a, b) {
					if (this.text && this.text.length > 0) {
						...
						a.fillStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")", a.fillText(this.text, d - f / 2, e - g / 2), a
	...
	VisualGraph.prototype.setZoom = function(type) {
		...
		if (type == 'zoomOut') {
			...
			self.animate({
				x:{from:self.scene.scaleX,to:1},
				y:{from:self.scene.scaleY,to:1}
			},
			1000,'easeInOutCubic',function(result){
				self.scene.scaleX = result.x;
				self.scene.scaleY = result.y;
			});
		} else {
